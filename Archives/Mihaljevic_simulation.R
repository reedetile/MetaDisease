...
#Next step: Density dependent saturated model
sum(K)
max_abund <- 65
...
a <- 6
b <- 4
error <- rnorm(length(species_richness), mean = 0, sd = 0.1)
...
# I think I have successfully done it! This again shows an amplification affect when using a saturated model for a density dependent system.
#This roughly matches Figure B(panel D) in the supplementals
...
for(c in 1:length(comms_saturated)){
  R <- rich_abund[c,1]
  KCOM <- max_abund/(1+50*exp(-0.05*(R)))
  KS <- KCOM/rich_abund[c,2]
